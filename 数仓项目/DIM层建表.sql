...
(
    `id`                 STRING COMMENT 'id',
    `pos_location`     STRING COMMENT '坑位位置',
    `pos_type`          STRING COMMENT '坑位类型 ',
...
-- 8.7 日期维度表 -- 手动插入
-- 手动插入时，需要先转为文本格式临时表然后再 insert ... select ... 插入到 orc 格式的表
...
show create table tmp_dim_date_info;
show create table dim_date;

select * from dim_activity_full limit 10;
select * from dim_coupon_full limit 10;
select * from dim_date limit 10;
select * from dim_promotion_pos_full limit 10;
select * from dim_promotion_refer_full limit 10;
select * from dim_province_full limit 10;
...
-- 将数据文件 date_info.txt 上传到HDFS上临时表路径/warehouse/gmall/tmp/tmp_dim_date_info
insert overwrite table dim_date select * from tmp_dim_date_info;
select * from dim_date limit 10;
select count(*) from dim_date; --365 条
